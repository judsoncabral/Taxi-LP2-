    ...
    public void showStatus()
    {
        ...
        for(Vehicle vehicle : vehicleFleet){
            if( vehicle instanceof Taxi ){
                System.out.println( "Taxi" + vehicle.getStatus() );
            }
            else if (vehicle instanceof Shuttle ){
                System.out.println( "Bus" + vehicle.getStatus() );
            }
            //System.out.println( vehicle.getStatus() );
        }
    }
    /**
     * Find one vehicle that could go to destination 
     */
    public Vehicle nextDestination( String destination )
    {
        //Search first in the list of vehicle the Shuttle(Bus) with destinatio equal and return if found
        for(Vehicle vehicle : vehicleFleet){
        ...
        //Search for the taxi without destination(It's free to go every where);
        for(Vehicle vehicle : vehicleFleet){
            if(vehicle instanceof Taxi){
                if( null == vehicle.getDestination()){
                    return vehicle;
                }
            }
        }
